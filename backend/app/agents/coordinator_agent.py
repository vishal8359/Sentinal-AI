...
def run_incident_workflow(incident_id: str, log_excerpt: str | None = None) -> dict:
    ...
    record_agent_run(incident_id, "Log Analysis", "done", log_excerpt or "sample log", json.dumps(log_output), 0.91)

    knowledge_output = run_knowledge_agent(log_output)
    record_agent_run(incident_id, "Knowledge", "done", "Runbook retrieval", json.dumps(knowledge_output), 0.88)

    root_cause_output = run_root_cause_agent(log_output, knowledge_output)
    record_agent_run(incident_id, "Root Cause", "done", "Ranked hypotheses", json.dumps(root_cause_output), root_cause_output["hypotheses"][0]["confidence"])

    remediation_output = run_remediation_agent(root_cause_output)
    record_agent_run(incident_id, "Remediation", "done", "Proposed actions", json.dumps(remediation_output), 0.9)

    # Record Coordinator as done with a human-readable summary (Bug 2 fix)
    top_hypothesis = root_cause_output["hypotheses"][0]
    coordinator_summary = json.dumps({
        "summary": f"Dispatched 4 agents. Root cause identified: {top_hypothesis['cause']} ({int(top_hypothesis['confidence'] * 100)}% confidence). Awaiting human approval.",
        "agents_dispatched": 4,
        "top_cause": top_hypothesis["cause"],
        "top_confidence": top_hypothesis["confidence"],
    })
    record_agent_run(incident_id, "Coordinator", "done", "Investigation complete", coordinator_summary, top_hypothesis["confidence"])
